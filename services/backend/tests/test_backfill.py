...
def _lire_backfill(storage):
    with storage.engine.connect() as conn:
        result = conn.execute(
            text(
                "SELECT source, payload FROM raw_readings "
                "WHERE source = 'api_backfill' ORDER BY id"
            )
        )
...
def test_stocke_les_mesures_telles_quelles(storage):
    mesures = [
        {"timestamp": "2024-09-01T00:00:00", "site_id": "SITE001",
         "consumption_kw": None, "null_reasons": ["network_loss"],
         "data_quality": "critical"},
        {"timestamp": "2024-09-01T00:01:00", "site_id": "SITE001",
         "consumption_kw": 70.9, "null_reasons": [], "data_quality": "good"},
    ]
...
def test_toutes_les_mesures_arrivent_en_base(storage):
    fenetre_1 = [{"timestamp": f"2024-09-01T00:{m:02d}:00", "site_id": "SITE001"}
                 for m in range(3)]
    fenetre_2 = [{"timestamp": f"2024-09-01T01:{m:02d}:00", "site_id": "SITE001"}
                 for m in range(2)]
