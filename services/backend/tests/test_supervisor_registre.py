"""Tests de la source de seuil MLflow : lire la MAE holdout d une version.

Aucun MLflow ici : les reponses de son API REST sont simulees. Ce qui est
verifie, c est le calcul (MAE x tolerance), les deux appels dans le bon ordre,
et surtout que tout echec rend None sans lever, pour que l ordonnanceur
continue avec le filet seul.
"""
...
def test_le_seuil_est_la_mae_holdout_fois_la_tolerance(registre):
...
def test_un_run_sans_mae_ne_donne_pas_de_seuil(registre, caplog):
    ...
    assert "pas de mae" in caplog.text
